package com.example.beautydate.data.local
...
interface ServiceDao {
    ...
    // Price bulk update methods (simplified for compilation)
    @Query("UPDATE services SET price = price * (1 + :percentage/100) WHERE businessId = :businessId AND category = :category")
    suspend fun increasePricesByCategory(businessId: String, category: String, percentage: Double)
    
    @Query("UPDATE services SET price = price * (1 + :percentage/100) WHERE businessId = :businessId AND id IN (:serviceIds)")
    suspend fun increasePricesByIds(businessId: String, serviceIds: List<String>, percentage: Double)
    
    @Query("UPDATE services SET price = price * (1 + :percentage/100) WHERE businessId = :businessId")
    suspend fun increaseAllPrices(businessId: String, percentage: Double)
    
    @Query("UPDATE services SET price = price * (1 - :percentage/100) WHERE businessId = :businessId AND category = :category")
    suspend fun decreasePricesByCategory(businessId: String, category: String, percentage: Double)
    
    @Query("UPDATE services SET price = price * (1 - :percentage/100) WHERE businessId = :businessId AND id IN (:serviceIds)")
    suspend fun decreasePricesByIds(businessId: String, serviceIds: List<String>, percentage: Double)
    
    @Query("UPDATE services SET price = price * (1 - :percentage/100) WHERE businessId = :businessId")
    suspend fun decreaseAllPrices(businessId: String, percentage: Double)
    
    @Query("UPDATE services SET price = price + :amount WHERE businessId = :businessId AND category = :category")
    suspend fun addFixedAmountByCategory(businessId: String, category: String, amount: Double)
    
    @Query("UPDATE services SET price = price + :amount WHERE businessId = :businessId AND id IN (:serviceIds)")
    suspend fun addFixedAmountByIds(businessId: String, serviceIds: List<String>, amount: Double)
    
    @Query("UPDATE services SET price = price + :amount WHERE businessId = :businessId")
    suspend fun addFixedAmountToAll(businessId: String, amount: Double)
    
    @Query("UPDATE services SET price = price - :amount WHERE businessId = :businessId AND category = :category")
    suspend fun subtractFixedAmountByCategory(businessId: String, category: String, amount: Double)
    
    @Query("UPDATE services SET price = price - :amount WHERE businessId = :businessId AND id IN (:serviceIds)")
    suspend fun subtractFixedAmountByIds(businessId: String, serviceIds: List<String>, amount: Double)
    
    @Query("UPDATE services SET price = price - :amount WHERE businessId = :businessId")
    suspend fun subtractFixedAmountFromAll(businessId: String, amount: Double)
} 